...
impl ClearInterruptState {
    ...
    fn try_execute(
        ...
        match read_state.advance(registers, memory) {
            None => {
                *self = ClearInterruptState::Fetching(read_state);
                false
            }
            Some(_) => {
                registers.status.set_interrupt(false);

                *self = ClearInterruptState::Executed;
                true
            }
        }
    }
...
mod tests {
    ...
    gen_imp_test!(0x58, assert());
    gen_imp_cycles_test!(0x58, 2);
}
